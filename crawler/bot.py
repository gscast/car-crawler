...
class ScrapingBotCAR:
    ...
    def __init__(self, uf, email, download_dir, debug=False):
        ...
        # keep up with the Captcha solving tries
        self.n_tries = 0
        self.MAX_TRIES = 80
        ...
        # define headless browser options
        options = Options()
        if not self.debug:
    ...
    def __call__(self):
        ...
        for city in tqdm.tqdm(cities):
            ...
            # skip file if the csv is donwloaded
            ...
            try:
                ...
                # refresh captcha and try until success
                self.n_tries = 0
    ...
    def __solve_captcha(self):
        """Screenshot the captcha, preprocess it, and pass it on to the OCR."""
        ...
        # crop to the capthca location
        rect = captcha_img.rect
        img = img.crop((rect["x"], rect["y"],
                        rect["x"] + rect["width"],
                        rect["y"] + rect["height"]))

        # save crop on debug mode
        if self.debug:
            img.save("crawler/.tmp/captcha.png")

        # remove the red stripes, and enhance
        img = process_img(img)
        # predict text with pytesseract character recognition.
